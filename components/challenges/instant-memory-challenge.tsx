  ...
  return (
    <div className="fixed inset-0 z-50 w-full h-full min-h-screen flex flex-col items-center p-4 md:p-8 overflow-hidden font-sans bg-[#FDFCF8] select-none touch-none">
      ...
      <button 
        ...
        className="absolute top-4 left-4 z-50 p-2 bg-white border-2 border-[#D4AF37]/20 rounded-full shadow-md text-gray-500 hover:text-red-500 hover:border-red-200 hover:bg-red-50 transition-all duration-300 group"
      >
      ...
      <div className="relative z-10 w-full max-w-4xl flex justify-between items-center mb-6 mt-8 md:mt-0">
        <div className="flex items-center gap-3 bg-white border border-[#D4AF37]/30 px-5 py-2.5 rounded-2xl shadow-sm">
            <Timer className={`w-6 h-6 ${timeLeft <= 10 ? 'text-red-500 animate-pulse' : 'text-[#D4AF37]'}`} />
        ...
        <div className="flex items-center gap-2">
            ...
            <div className="flex items-center justify-center w-10 h-10 bg-[#D4AF37] text-white rounded-xl shadow-md">
      ...
      <div className="relative z-10 w-full max-w-2xl mb-8">
        <div className="flex flex-col w-full">
            <div className="flex justify-between items-end mb-2 px-2">
                 ...
                 <h3 className="text-xl font-bold text-[#D4AF37] transition-all duration-300 select-none">
                    {phase === 'memorize' ? 'احفظ أماكن الأشكال' : 'أعد ترتيب الأشكال الآن'}
                 </h3>
                 {/* Timer count only shows in memorize phase */}
                 {phase === 'memorize' && (
                    <span className="font-mono text-xl font-bold text-[#D4AF37] animate-pulse">
                        {memorizeTime}ث
                    </span>
                 )}
            </div>
            ...
            {phase === 'memorize' && (
              <div className="w-full h-3 bg-gray-200 rounded-full overflow-hidden shadow-inner animate-in fade-in duration-300">
                  <div 
                      className="h-full transition-all duration-1000 ease-linear rounded-full bg-[#D4AF37]"
                      style={{ 
      ...
      <div className="relative z-10 flex-1 w-full max-w-4xl flex flex-col items-center justify-start gap-8">
        ...
        {/* Memory Phase */}
        {phase === "memorize" && (
          <div className="w-full bg-white border-2 border-[#D4AF37]/10 rounded-3xl p-8 md:p-12 shadow-xl flex items-center justify-center animate-in fade-in zoom-in duration-500">
        ...
        {phase === "recall" && (
          <div className="w-full flex flex-col gap-6 animate-in slide-in-from-bottom-10 fade-in duration-500">
            ...
            {/* DROP ZONES */}
            <div className="bg-white border-2 border-[#D4AF37]/10 rounded-3xl p-6 md:p-8 shadow-lg">
                ...
                {placedItems.map((placedItem, index) => (
                  <div
                    ...
                    className={`
                        ...
                        ${placedItem ? 'bg-white' : 'bg-gray-50 border-2 border-dashed border-[#D4AF37]/30'}
                        ${activeDropZone === index ? 'border-2 border-[#D4AF37] bg-[#D4AF37]/10 scale-105' : ''}
                    `}
                  >
                    {placedItem ? (
                      <div 
                      ...
                      <span className="text-3xl font-bold text-[#D4AF37]/20 select-none font-mono">{index + 1}</span>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white border-t-4 border-[#D4AF37] rounded-t-3xl md:rounded-3xl p-6 pb-20 md:pb-6 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
